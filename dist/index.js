 ...
 */
const checkVersionLag = (npmData, installedVersion) => {
    if (!(npmData === null || npmData === void 0 ? void 0 : npmData.versions))
    ...
    const allVersions = Object.keys(npmData.versions)
        .filter((v) => {
        var _a;
        const coerced = (0, semver_1.coerce)(v);
        const isDeprecated = (_a = npmData.versions[v]) === null || _a === void 0 ? void 0 : _a.deprecated;
        return coerced && !(0, semver_1.prerelease)(v) && !isDeprecated;
    })
        .sort((a, b) => {
        const coercedA = (0, semver_1.coerce)(a);
        const coercedB = (0, semver_1.coerce)(b);
        if (!coercedA || !coercedB)
            return 0;
        return (0, semver_1.compare)(coercedA, coercedB);
    });
    ...
    if (!latestVersion)
        return { reason: undefined, severity: undefined };
    const installedCoerced = (0, semver_1.coerce)(installedVersion);
    const latestCoerced = (0, semver_1.coerce)(latestVersion);
    if (!installedCoerced || !latestCoerced)
        return { reason: undefined, severity: undefined };
    const installedMajor = (0, semver_1.major)(installedCoerced);
    const latestMajor = (0, semver_1.major)(latestCoerced);
    const installedMinor = (0, semver_1.minor)(installedCoerced);
    const latestMinor = (0, semver_1.minor)(latestCoerced);
 ...
 */
const checkOsvRange = (normalizedVersion, range) => {
    var _a, _b;
    ...
    const introduced = (_a = range.events.find((event) => event.introduced)) === null || _a === void 0 ? void 0 : _a.introduced;
    const fixed = (_b = range.events.find((event) => event.fixed)) === null || _b === void 0 ? void 0 : _b.fixed;
    ...
    if (normalizedIntroduced && fixed) {
        ...
        try {
            return (0, semver_1.satisfies)(normalizedVersion, rangeString);
        }
        catch (_c) {
...
const isVersionAffected = (vuln, version) => {
    var _a;
    ...
    if (!normalizedVersion)
        return true; // If we can't parse, assume affected (safe default)
    ...
    return vuln.affected.some((affected) => {
        ...
        if ((_a = affected.versions) === null || _a === void 0 ? void 0 : _a.includes(version)) {
            return true;
        }
        // Check version ranges
        if (affected.ranges) {
            return affected.ranges.some((range) => checkOsvRange(normalizedVersion, range));
        }
        return false;
    });
};
...
const checkVulnerabilities = (packageName, version) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        if (data.vulns && data.vulns.length > 0) {
            ...
            const affectingVulns = data.vulns.filter((vuln) => isVersionAffected(vuln, cleanVersion));
            if (affectingVulns.length > 0) {
                const vulnLinks = affectingVulns
                    .slice(0, 3)
                    .map((v) => `[${v.id}](https://osv.dev/vulnerability/${v.id})`)
                    .join(', ');
                const suffix = affectingVulns.length > 3
        ...
        if (advisories && advisories.length > 0) {
            ...
            const affectingAdvisories = advisories.filter((advisory) => {
                ...
                return advisory.vulnerabilities.some((vuln) => {
                    ...
                    if (((_a = vuln.package) === null || _a === void 0 ? void 0 : _a.ecosystem) && vuln.package.ecosystem !== 'npm') {
                        return false;
                    }
                    // Check version range
                    return isVersionAffectedByAdvisory(cleanVersion, vuln.vulnerable_version_range);
                });
            });
            if (affectingAdvisories.length > 0) {
                const advisoryLinks = affectingAdvisories
                    ...
                    .join(', ');
                const suffix = affectingAdvisories.length > 3
...
const analyzeRisk = (npmData, packageName, version, repoUrl) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (versionLag.reason)
        reasons.push(versionLag.reason);
    if (vulnResult)
 ...
 */
const findFilesRecursive = (directory, resultArray) => {
    node_fs_1.default.readdirSync(directory).forEach((file) => {
        const subpath = path.join(directory, file);
        if (node_fs_1.default.lstatSync(subpath).isDirectory()) {
