...
`
  ...
  const rows = vulns
    .map(
      (v) =>
        `| ${v.name} | ${v.version} | ${v.dependencyPath} | ${v.severity} | ${
          v.advisory
        } | ${v.recommendation} | ${v.type} | ${escapeTableCell(
          v.verification
        )} |`
    )
    .join('\n')
...
const generateSoupTable = (soupData: TSoupData[]) => {
  ...
  soupData.forEach((data) => {
    tableContents.push(
      `| ${data.soupName} | ${data.soupLanguages} | ${data.soupSite} | ${
        data.soupLicense
      } | ${data.soupVersion} | ${data.soupRiskLevel} | ${
        data.soupRiskDetails
      } | ${escapeTableCell(data.soupVerification)} |`
    )
  })
...
/**
 * Format SOUP.md using prettier if available or installable
 * @param soupPath string: absolute path to SOUP.md
 */
const formatSoupFile = (soupPath: string): void => {
  const cwd = path.dirname(soupPath)

  try {
    // Check if prettier is available in the consumer's node_modules
    execSync('npx prettier --version', {
      encoding: 'utf8',
      cwd,
      stdio: 'pipe',
    })
    core.info('💅 Formatting SOUP.md with prettier...')
    execSync(`npx prettier --write "${soupPath}"`, {
      encoding: 'utf8',
      cwd,
      stdio: 'pipe',
    })
    core.info('✅ SOUP.md formatted')
  } catch {
    // prettier not available via npx, install temporarily
    core.info(
      '💅 prettier not found locally, installing temporarily to format SOUP.md...'
    )
    try {
      execSync('npm install --no-save prettier', {
        encoding: 'utf8',
        cwd,
        stdio: 'pipe',
      })
...
const generateSoupRegister = async () => {
  ...
  // Format SOUP.md if enabled
  const shouldFormat = core.getInput('format') !== 'false'
  if (shouldFormat) {
    formatSoupFile(soupPath)
  }
  ...
  if (createPr) {
    if (hasSoupChanges(soupPath)) {
      ...
      const customPrBody = core.getInput('pr-body') || ''
